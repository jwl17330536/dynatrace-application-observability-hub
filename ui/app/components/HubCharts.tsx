...
const HEX = {
  ...
  primary: "#3d6df2",
  muted: "#6b7280",
  series: ["#3d6df2", "#2c8b57", "#c9842b", "#c33c54", "#7c5cbf", "#0d9488", "#db2777"],
} as const;
...
function resolveColor(color: string | undefined, index: number): string {
  ...
  return HEX.series[index % HEX.series.length];
}
  ...
  return (
    <div style={{ display: "grid", gap: "8px", padding: "4px 0", width: "100%" }}>
      {filtered.map((row, index) => {
        ...
        return (
          <div key={`${row.category}-${index}`} style={{ display: "grid", gap: "4px" }}>
...
export function HubHorizontalBarChart({
  ...
  return <HtmlBarList data={filtered} emptyMessage={emptyMessage} />;
}
  ...
  return (
    <div style={{ display: "grid", gap: "10px", padding: "4px 0", width: "100%" }}>
      {filtered.map((row) => {
        ...
        return (
          <div key={row.category} style={{ display: "grid", gap: "4px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: "8px", fontSize: "12px" }}>
              <span style={{ color: theme.text, fontWeight: 600 }} title={row.category}>
                {truncateLabel(row.category)}
              </span>
                ...
                return (
                  <div
                    ...
                    style={{
                      width: `${segPct}%`,
                      height: "100%",
                      backgroundColor: severityColors[key] || HEX.primary,
                    }}
                  />
                );
              })}
...
export const hubChartColors = HEX;
